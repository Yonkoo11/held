...
- [ ] Blocky402 facilitator account/API key — needed for: the Hedera track requires the x402 gating to run **via Blocky402** specifically, not any x402 implementation — how: https://blocky402.com/ — cost: unknown, assume free testnet tier — eta: unknown, probe first — fallback: self-hosted x402 facilitator against Hedera testnet using the reference PoC at github.com/hedera-dev/x402-inference-pay-per-request-poc; the flow is identical and still on Hedera, but the submission must say plainly that Blocky402 was not used, which likely costs the track.
