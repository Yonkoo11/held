...
#
#   nohup sh scripts/serve.sh > /tmp/outcomelock-serve.log 2>&1 &
#
...
while true; do
  if ! curl -s --max-time 6 -o /dev/null "http://127.0.0.1:$PORT/health"; then
    ...
    DEMO_BUY="${DEMO_BUY:-on}" REVIEW_MINUTES="${REVIEW_MINUTES:-20}" \
      nohup npm run --silent seller > /tmp/outcomelock-seller.log 2>&1 &
    sleep 12
  fi
  ...
  if [ -z "$URL" ]; then
    ...
    sleep 2
    nohup ngrok http "$PORT" --log stdout > /tmp/outcomelock-ngrok.log 2>&1 &
    sleep 12
    URL=$(curl -s --max-time 6 http://127.0.0.1:4040/api/tunnels 2>/dev/null \
          | sed -n 's/.*"public_url":"\(https:[^"]*\)".*/\1/p' | head -1)
  fi
